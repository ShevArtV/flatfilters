...
export default class GetList {
    ...
    responseSuccess(response, form) {
        if (response.object.action === 'mgr/discount/duplicate') {
            if (response.object.html) {
                bootstrap.Modal.getInstance(this.config.copy_modal).hide();
                this.config.rows_wrapper.innerHTML += response.object.html;
                const forms = this.config.rows_wrapper.querySelectorAll(this.config.row_prefix+response.object.id+' form:not(.js_form_active)');
                const activateForm = this.config.rows_wrapper.querySelector(this.config.row_prefix+response.object.id+' .js_form_active');
                if (forms.length) {
                    this.setFormHandler(forms);
                }
                if(activateForm){
                    this.addListenerToActiveBtn(activateForm);
                }
            }
        }
    }
